...
static inline long diff_nsec(struct timespec befor, struct timespec after)
{
	return ((after.tv_sec * NSECS_PER_SEC + after.tv_nsec)
			- (befor.tv_sec * NSECS_PER_SEC + befor.tv_nsec));
}

static unsigned long loops_per_msec()
{
	struct timespec befor, after;
	clock_gettime(CLOCK_MONOTONIC, &befor);
	...
	int ret;
	return NLOOP_FOR_ESTIMATION * NSECS_PER_MSEC / diff_nsec(befor, after);
}
...
int main(int argc, char *argv[])
{
	...
	if( nproc < 1)
	{
		fprintf( stderr, "<nproc>(%d) shold be >= 1\n", nproc);
		exit(EXIT_FAILURE);
	}

	if(total < 1)
	{
		fprintf( stderr, "<total>(%d) shold be >= 1\n", total);
		exit(EXIT_FAILURE);
	}
	
	if(resol < 1)
	{
		fprintf( stderr, "<resol>(%d) shold be >= 1\n", resol);
		exit(EXIT_FAILURE);
	}
	...
	if( !logbuf )
		err(EXIT_FAILURE, "mallock(logbuf) failed.\n");
